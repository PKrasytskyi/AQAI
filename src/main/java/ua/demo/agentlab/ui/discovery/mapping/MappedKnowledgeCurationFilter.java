...
public class MappedKnowledgeCurationFilter {

    private static final double MIN_PROMOTED_SCORE = 0.75d;

    public MappedUiKnowledge filterForCuration(MappedUiKnowledge knowledge) {
        if (knowledge == null) {
            return null;
        }
    ...
    private MappedElement filterElement(MappedElement element) {
        return new MappedElement(
                ...
                element.clickable(),
                element.visible(),
                safeCandidates(element.locatorCandidates()),
    ...
    private MappedField filterField(MappedField field) {
        return new MappedField(
                ...
                field.required(),
                field.placeholder(),
                safeCandidates(field.locatorCandidates())
        );
    }

    private List<LocatorCandidate> safeCandidates(List<LocatorCandidate> candidates) {
        return candidates.stream()
                .filter(this::isSafeCandidate)
                .sorted(Comparator.comparingDouble(LocatorCandidate::stabilityScore).reversed())
                .limit(3)
                .map(this::demoteToCandidate)
                .toList();
    }

    private boolean isSafeCandidate(LocatorCandidate candidate) {
    ...
    private LocatorCandidate demoteToCandidate(LocatorCandidate candidate) {
        return new LocatorCandidate(candidate.strategy(), candidate.value(), candidate.stabilityScore(),
                candidate.evidenceSource(), candidate.elementRole(), candidate.accessibleName(),
                candidate.visibleText(), candidate.href(), candidate.originHost(), candidate.sameOrigin(),
                candidate.uniqueOnPage(), candidate.stableAcrossRuns(), candidate.risks(),
