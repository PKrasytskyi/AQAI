...
public class SliderCapabilityAdapter implements BrowserCapabilityAdapter {
    ...
    public BrowserCapabilityResult execute(WebDriver driver, BrowserCapabilityRequest request) {
        ...
        double min = number(slider.getDomAttribute("min"), 0.0d);
        double step = Math.max(number(slider.getDomAttribute("step"), 1.0d), 0.0001d);
        ...
        double actual = number(slider.getDomProperty("value"), Double.NaN);
