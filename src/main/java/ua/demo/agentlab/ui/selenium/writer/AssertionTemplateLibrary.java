...
public class AssertionTemplateLibrary {
    ...
    public String buildAssertionBlock(UiOperationType operationType, UiScenarioTemplateContext context) {
        ...
        return switch (operationType) {
            case PAGE_OPEN -> positiveStateAssertion(
                    context,
                    "Expected target page to be opened successfully"
            );
            ...
            case FLOW_NAVIGATION -> positiveStateAssertion(
                    context,
                    "Expected navigation target to be opened"
            );
            ...
            case ENTITY_DELETE -> positiveStateAssertion(
                    context,
                    "Expected entity to be deleted successfully"
            );
            case SESSION_END -> positiveStateAssertion(
                    context,
                    "Expected logout flow to complete successfully"
            );
            case FILE_UPLOAD -> positiveStateAssertion(
                    context,
                    "Expected file upload to complete successfully"
            );
    ...
    private String containerItemAssertion(UiScenarioTemplateContext context) {
        return """
                ...
                Assert.assertTrue(
                        ...
                        "Expected selected item to be present in the target container"
                );
                """.formatted(
                context.dataSetName(),
                context.pageVariableName()
    ...
    private String containerEmptyAssertion(UiScenarioTemplateContext context) {
        return """
                Assert.assertTrue(
                        ...
                        "Expected target container to be empty"
                );
                """.formatted(
                context.pageVariableName()
        );
    ...
    private String urlContainsAssertion(UiScenarioTemplateContext context, String fragment) {
        return """
                Assert.assertTrue(
                        %s.getCurrentUrl().contains("%s"),
                        "Expected current URL to contain: %s"
                );
                """.formatted(
                context.pageVariableName(),
                fragment,
