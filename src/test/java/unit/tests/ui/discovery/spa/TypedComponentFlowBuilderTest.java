...
class TypedComponentFlowBuilderTest {
    @Test void derivesNavigationAndTableFlowsFromTypedComponentEvidence() {
        ...
        SpaPageInventory page = new SpaPageInventory("dashboard","DashboardPage","/dashboard","AUTHENTICATED_AREA","fp",null,List.of(nav,table),List.of());
        var flows = new TypedComponentFlowBuilder().build(new SpaInventoryBundle("v1", ua.demo.agentlab.ui.discovery.spa.SpaDiscoveryMode.INVENTORY,List.of(page),List.of())).flows();
