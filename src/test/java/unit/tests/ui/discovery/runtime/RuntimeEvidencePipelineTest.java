...
public class RuntimeEvidencePipelineTest {
    ...
    public void bidiNormalizerBuildsRuntimeEvidenceFromLiveBufferedEvents() {
        List<BiDiRuntimeEvent> events = List.of(
                ...
                new BiDiRuntimeEvent(
                        ...
                        "dashboard-index",
                        "https://app.test/dashboard/index",
                        Map.of("inflight", "0", "idleForMs", "500"),
                        5L
                ),
                new BiDiRuntimeEvent(
                        "browsingContext.lifecycle",
                        "dashboard-index",
                        "https://app.test/dashboard/index",
                        Map.of("name", "load", "readyState", "complete"),
                        6L
                )
        );
        ...
        Assert.assertTrue(bundle.stateTransitions().get(0).confidence() >= 0.90d);
        Assert.assertTrue(bundle.sourceTrace().stream().anyMatch(trace -> trace.equals("networkIdleEvents=1")));
        Assert.assertTrue(bundle.sourceTrace().stream().anyMatch(trace -> trace.equals("lifecycleEvents=1")));
    }

    @Test
    public void bidiCollectorPreservesLiveLoginToDashboardRuntimeChain() {
        BiDiEventBuffer buffer = new BiDiEventBuffer(50);
        buffer.add(new BiDiRuntimeEvent(
                "bidi.session.start",
                "auth-login",
                "https://opensource-demo.orangehrmlive.com/web/index.php/auth/login",
                Map.of("url", "https://opensource-demo.orangehrmlive.com/web/index.php/auth/login"),
                1L
        ));
        buffer.add(new BiDiRuntimeEvent(
                "network.requestWillBeSent",
                "auth-login",
                "https://opensource-demo.orangehrmlive.com/web/index.php/auth/login",
                Map.of("method", "POST", "url", "https://opensource-demo.orangehrmlive.com/web/index.php/auth/validate", "resourceType", "xhr"),
                2L
        ));
        buffer.add(new BiDiRuntimeEvent(
                "network.responseCompleted",
                "auth-login",
                "https://opensource-demo.orangehrmlive.com/web/index.php/auth/login",
                Map.of("method", "POST", "url", "https://opensource-demo.orangehrmlive.com/web/index.php/auth/validate", "status", "200", "resourceType", "xhr"),
                3L
        ));
        buffer.add(new BiDiRuntimeEvent(
                "browsingContext.navigationStarted",
        ...
        RuntimeEvidenceBundle bundle = new BiDiRuntimeEvidenceCollector(buffer).collect(null);

        Assert.assertEquals(bundle.networkRequests().size(), 1);
        Assert.assertEquals(bundle.networkResponses().size(), 1);
        Assert.assertEquals(bundle.semanticNetworkEvidence().size(), 1);
        Assert.assertEquals(bundle.semanticNetworkEvidence().get(0).operation(), "AUTHENTICATE");
        Assert.assertEquals(bundle.stateTransitions().size(), 1);
        Assert.assertEquals(bundle.stateTransitions().get(0).fromUrl(),
                "https://opensource-demo.orangehrmlive.com/web/index.php/auth/login");
        Assert.assertEquals(bundle.stateTransitions().get(0).toUrl(),
                "https://opensource-demo.orangehrmlive.com/web/index.php/dashboard/index");
        Assert.assertEquals(bundle.stateTransitions().get(0).transitionType(), "SPA_ROUTE_CHANGE");
        Assert.assertTrue(bundle.stateTransitions().get(0).sourceTrace().contains("networkIdle=true"));
        Assert.assertTrue(bundle.sourceTrace().stream().anyMatch(trace -> trace.equals("mode=bidi-buffer")));
    }
    ...
    @Test
    public void runtimeFeedbackDeduplicatesUnstableLocatorReviewIssues() {
        PageLocatorModel repeated = new PageLocatorModel(
                "css",
                "button[type='submit']",
                0.88d,
                "submit button",
        ...
        PageLocatorModel noisyLowValue = new PageLocatorModel(
                "css",
                "div > div > div > button",
                0.30d,
                "hierarchy",
        ...
        PageModel page = new PageModel(
                ...
                new PageEvidenceModel("", ""),
                List.of(
                        element("submit-1", repeated, noisyLowValue),
                        element("submit-2", repeated)
                ),
                List.of(),
                List.of(),
                List.of()
        ...
        var summary = new RuntimeFeedbackAnalyzer().analyze(new PageModelBundle(List.of(page)), runtimeBundle());

        Assert.assertEquals(summary.unstableLocators(), 3);
        Assert.assertEquals(
                summary.issues().stream().filter(issue -> issue.issueType().equals("UNSTABLE_LOCATOR")).count(),
                1
        );
        Assert.assertTrue(summary.issues().stream().anyMatch(issue -> issue.evidence().equals("css=button[type='submit']")));
    }

    private PageElementModel element(String id, PageLocatorModel... locators) {
        return new PageElementModel(
                ...
                Map.of(),
                List.of(locators),
                locators.length == 0 ? null : locators[0],
                List.of(),
                0.90d
        );
    }
