...
public class LlmOutputSchemaValidatorTest {
    ...
    @Test
    public void pomContractAcceptsStructuredStepsWithoutJavaBody() throws Exception {
        var report = validator.validatePomContract(objectMapper.readTree("""
                {
                  "schemaVersion": "pom-contract-v1",
                  "page": {
                    "name": "LoginPage",
                    "route": "/login",
                    "capability": "AUTHENTICATION_FORM",
                    "openMethod": "openLogin"
                  },
                  "locators": [
                    {
                      "id": "usernameInput",
                  ...
                  "assertions": [
                    {
                      "methodName": "isUsernameFieldVisible",
                      "returnType": "boolean",
                      "checks": [
    ...
    @Test
    public void pomContractRejectsJavaBodyShape() throws Exception {
        var report = validator.validatePomContract(objectMapper.readTree("""
                {
                  "schemaVersion": "pom-contract-v1",
                  "page": {"name": "LoginPage", "route": "/login", "capability": "AUTHENTICATION_FORM", "openMethod": "openLogin"},
                  "locators": [],
                  "actions": [
                    {
                      "methodName": "clickLoginButton",
                      "parameters": [],
                      "body": "elements.click(loginButton);"
                    }
                  ],
                  "assertions": [],
                  "coverageGaps": [],
        ...
        Assert.assertFalse(report.valid());
        Assert.assertTrue(report.issues().stream().anyMatch(issue -> issue.path().contains(".steps")));
    }
