...
> **Status snapshot, reviewed 2026-07-15.** For current setup, supported run modes,
> artifact review, and branch delivery rules, use
> [Developer Onboarding and Delivery Guide](DEVELOPER_ONBOARDING.md).
...
The stable golden UI workflow is `requirements/valid-login-requirement.md`.
The active OrangeHRM project profile may point to a different requirement fixture
for feature stabilization; that does not change the release baseline until its
own compile/review/live-smoke evidence is green.
...
The checked-in `framework.properties` is demo-oriented and enables AI/RAG/knowledge-store switches:
...
Disable them through JVM properties for clean deterministic or no-DB comparison runs.
...
- external AI/RAG/DB calls still require environment secrets and reachable local services.
