...
The active UI path is now centered around canonical test cases and shared page-object contracts.
...
flowchart LR
    ...
    H --> I["UiTestPlanAgent"]
    I --> J["UI scenarios"]
    J --> K["SharedPageObjectContractAggregator"]
    K --> L["Shared Page Objects"]
    J --> M["UI test writer"]
    L --> N["GeneratedUiContractValidationAgent"]
    M --> N
    N --> O["GeneratedCodeCompileAgent"]
    O --> P["GeneratedCodeReviewAgent"]
```
...
- `RuleBasedRequirementToTestCaseGenerator`
...
The system is stronger than before, but still not yet “product-finished”.
